...
for p, nrNodes in enumerate(parameters):
    ...
    for rep in range(repetitions):
        ...
        nrCollisions = sum(
            [
                ...
                for p in packets
                for n in nodes
                if p.collidedAtN[n.nodeid] == True
            ]
        )
        nrSensed = sum(
            [
        ...
        nrReceived = sum(
            [
                ...
                for p in packets
                for n in nodes
                if p.receivedAtN[n.nodeid] == True
            ]
        )
        ...
        if messageSeq != 0:
            nodeReach[rep] = (
                nrUseful / (messageSeq * (conf.NR_NODES - 1)) * 100
            )
        else:
            nodeReach[rep] = np.NaN
        if nrReceived != 0:
            nodeUsefulness[rep] = (
                nrUseful / nrReceived * 100
            )  # nr of packets that delivered to a message to a new receiver out of all packets received
        else:
            nodeUsefulness[rep] = np.NaN
        meanDelay[rep] = np.nanmean(delays)
        meanTxAirUtilization[rep] = (
            sum([n.txAirUtilization for n in nodes]) / conf.NR_NODES
        )
    if SAVE:
    ...
    print(
        "Tx air utilization average:",
        round(np.nanmean(meanTxAirUtilization), 2),
    )
