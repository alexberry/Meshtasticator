...
class interactiveNode:
    ...
    def setConfig(self):
        ...
        if self.hopLimit != 3:
            ...
            setattr(
                self.iface.localNode.localConfig.lora,
                "hop_limit",
                self.hopLimit,
            )
            p = admin_pb2.AdminMessage()
...
class interactiveGraph(Graph):
    ...
    def plotRoute(self, messageId):
        if self.firstTime:
            print(
                "Hover over an arc to show some info and click to remove it afterwards."
            )
        ...
        if len(packets) > 0:
            ...
            pairs = dict.fromkeys(
                list(set(p.transmitter for p in packets)), []
            )
            for p in packets:
                tx = p.transmitter
                rxs = p.receivers
                ...
                for ri, rx in enumerate(rxs):
                    ...
                    found = False
                    for pi, rxPair in enumerate(
                        pairs.get(tx)
                    ):  # pair is rx.nodeid and its count for this transmitter
                    ...
                    if p.packet["from"] == tx.hwId:
                        ...
                        if (
                            p.packet["decoded"]["simulator"]["portnum"]
                            == "ROUTING_APP"
                        ):
                    ...
                    origSenderField = "Original sender: " + str(
                        p.packet["from"] - HW_ID_OFFSET
                    )
                    fields.append(origSenderField)
                    destField = "Destination: " + to
                    fields.append(destField)
                    portNumField = "Portnum: " + str(
                        p.packet["decoded"]["simulator"]["portnum"]
                    )
            ...
            self.fig.canvas.draw_idle()
            self.fig.suptitle(
                "Route of message " + str(messageId) + " and ACKs"
            )
        else:
        ...
        if any(len(n.timestamps) > 1 for n in nodes):
            ...
            for n in nodes:
                if len(n.timestamps) > 0:
                    ...
                    plt.plot(
                        [t - initTime for t in n.timestamps],
                        n.channelUtilization,
                        label=str(n.nodeid),
                        marker=".",
                    )
...
class interactiveSim:
    def __init__(self):
        ...
        if self.docker:
            try:
                ...
                print(
                    "Please install the Docker SDK for Python with 'pip3 install docker'."
                )
                exit(1)
            n0 = self.nodes[0]
            ...
            if sys.platform == "darwin":
                self.container = dockerClient.containers.run(
                    ...
                    ports=dict(
                        zip(
                            (str(n.TCPPort) + "/tcp" for n in self.nodes),
                ...
                for n in self.nodes[1:]:
                    self.container.exec_run(
                        "./meshtasticd_linux_amd64 -e -d /home/node"
                        + str(n.nodeid)
                        + " -h "
                        + str(n.hwId)
                        + " -p "
                ...
                self.container = dockerClient.containers.run(
                    ...
                    user="root",
                    volumes={
                        "Meshtasticator": {"bind": "/home/", "mode": "rw"}
                    },
                )
                for n in self.nodes[1:]:
                    self.container.exec_run(
                ...
                print(
                    "You can check the device logs using 'docker exec -it "
                    + str(self.container.name)
                    + " cat /home/out_x.log', where x is the node number."
                )
        else:
            ...
            if which("gnome-terminal") is not None:
                ...
                print(
                    "The interactive simulator on native Linux (without Docker) requires either gnome-terminal or xterm."
                )
                exit(1)
            for n in self.nodes:  # [1:]
                if not xterm:
                    newTerminal = (
                        "gnome-terminal --title='Node "
                        + str(n.nodeid)
                        + "' -- "
                    )
                else:
                    newTerminal = (
                        "xterm -title 'Node " + str(n.nodeid) + "' -e "
                    )
                startNode = (
                    "program -d "
                    + os.path.expanduser("~")
        ...
        if self.forwardToClient:
            print(
                "Please connect with the client to TCP port",
                TCP_PORT_CLIENT,
                "...",
            )
            ...
            iface0 = tcp_interface.TCPInterface(
                hostname="localhost",
                portNumber=self.nodes[0].TCPPort,
                connectNow=False,
            )
            self.nodes[0].addInterface(iface0)
            iface0.myConnect()  # setup socket
            self.nodeThread = threading.Thread(
                target=self.nodeReader, args=(), daemon=True
            )
            self.clientThread = threading.Thread(
                target=self.clientReader, args=(), daemon=True
            )
        ...
        try:
            for n in self.nodes[int(self.forwardToClient) :]:
                iface = tcp_interface.TCPInterface(
                    hostname="localhost", portNumber=n.TCPPort
                )
            ...
            pub.subscribe(
                self.onReceiveMetrics, "meshtastic.receive.telemetry"
            )
            if self.forwardToClient:
    ...
    def parseInteractiveArgs(self, foundNodes):
        ...
        parser.add_argument(
            "nrNodes", type=int, nargs="?", choices=range(0, 11), default=0
        )
        ...
        parser.add_argument(
            "-p", "--program", type=str, default=os.getcwd() + "/"
        )
    ...
    def forwardPacket(self, receivers, packet, rssis, snrs):
        ...
        if "wantResponse" in packet["decoded"]:
            meshPacket.decoded.want_response = packet["decoded"][
                "wantResponse"
            ]
    ...
    def copyPacket(self, packet):
        ...
        try:
            if (
                "simulator" in packet
                or packet["decoded"]["portnum"] == "SIMULATOR_APP"
            ):
                return None
    ...
    def sendDM(self, text, fromNode, toNode):
        self.getNodeIfaceById(fromNode).sendText(
            text, destinationId=self.nodeIdToHwId(toNode), wantAck=True
        )
    ...
    def sendPing(self, fromNode, toNode):
        ...
        self.getNodeIfaceById(fromNode).sendData(
            payload,
            destinationId=self.nodeIdToHwId(toNode),
    ...
    def requestPosition(self, fromNode, toNode):
        self.getNodeIfaceById(fromNode).sendPosition(
            destinationId=self.nodeIdToHwId(toNode), wantResponse=True
        )
    ...
    def sendFromTo(self, fromNode, toNode):
        return self.getNodeIfaceById(fromNode).getNode(
            self.nodeIdToDest(toNode)
        )
    ...
    def onReceive(self, interface, packet):
        if "requestId" in packet["decoded"]:
            ...
            existingMsgId = next(
                (
                    ...
                    if m.packet["id"] == packet["decoded"]["requestId"]
                ),
                None,
            )
        ...
        if self.script:
            print(
                "Node",
                interface.myInfo.my_node_num - HW_ID_OFFSET,
                "sent",
                packet["decoded"]["simulator"]["portnum"],
                "with id",
                mId,
                "over the air!",
            )
        ...
        transmitter = next(
            (n for n in self.nodes if n.TCPPort == interface.portNumber), None
        )
        if transmitter is not None:
            receivers = [
                n for n in self.nodes if n.nodeid != transmitter.nodeid
            ]
    ...
    def onReceiveMetrics(self, interface, packet):
        fromNode = next(
            (n for n in self.nodes if n.hwId == packet["from"]), None
        )
        if fromNode is not None:
            ...
            if "deviceMetrics" in telemetryDict:
                ...
                if "time" in telemetryDict:
                    ...
                    if (
                        len(fromNode.timestamps) == 0
                        or timestamp > fromNode.timestamps[-1]
                    ):
    ...
    def onReceiveAll(self, interface, packet):
        if interface.portNumber == 4403:
            ...
            if fromRadio is not None:
                ...
                header = bytes(
                    [0x94, 0xC3, (bufLen >> 8) & 0xFF, bufLen & 0xFF]
                )
...
class CommandProcessor(cmd.Cmd):
    def cmdloop(self, sim):
        self.sim = sim
        print(
            "Type 'help' to list the available commands for sending messages. Type 'plot' to show the routes or 'exit' to exit the simulator."
        )
    ...
    def do_traceroute(self, line):
        ...
        print(
            "Instructing node",
            fromNode,
            "to send traceroute request to node",
            toNode,
            "(message ID =",
            str(self.sim.messageId + 1) + ")",
        )
        print(
            "This takes a while, the result will be in the log of node "
            + str(fromNode)
            + "."
        )
    ...
    def do_reqPos(self, line):
        ...
        print(
            "Instructing node",
            fromNode,
            "to send position request to node",
            toNode,
            "(message ID =",
            str(self.sim.messageId + 1) + ")",
        )
