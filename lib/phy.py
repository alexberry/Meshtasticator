...
slotTime = (
    8.5 * (2.0 ** conf.SFMODEM[conf.MODEM]) / conf.BWMODEM[conf.MODEM] * 1000
    + 0.2
    + 0.4
    + 7
)
...
def checkcollision(env, packet, rx_nodeId, packetsAtN):
    ...
    if packetsAtN[rx_nodeId]:
        for other in packetsAtN[rx_nodeId]:
            if frequencyCollision(packet, other) and sfCollision(
                packet, other
            ):
...
def airtime(sf, cr, pl, bw):
    ...
    payloadSymbNB = 8 + max(
        math.ceil(
            (8.0 * pl - 4.0 * sf + 28 + 16 - 20 * H) / (4.0 * (sf - 2 * DE))
        )
        * (cr + 4),
        0,
    )
...
def estimatePathLoss(dist, freq, txZ=conf.HM, rxZ=conf.HM):
    ...
    # Okumura-Hata model
        ...
        if conf.MODEL == 1:
            ahm = (
                1.1 * (math.log10(freq) - math.log10(1000000)) - 0.7
            ) * txZ - (1.56 * (math.log10(freq) - math.log10(1000000)) - 0.8)
        ...
        # rural area
            ...
            C = (
                -4.78 * ((math.log10(freq) - math.log10(1000000)) ** 2)
                + 18.33 * (math.log10(freq) - math.log10(1000000))
                - 40.98
            )

        A = (
            69.55
            + 26.16 * (math.log10(freq) - math.log10(1000000))
            - 13.82 * math.log(rxZ)
            - ahm
        )
    ...
    # 3GPP model
        ...
        Lpl = (
            (44.9 - 6.55 * math.log10(rxZ))
            * (math.log10(dist) - math.log10(1000))
            + 45.5
            + (35.46 - 1.1 * txZ) * (math.log10(freq) - math.log10(1000000))
            - 13.82 * math.log10(txZ)
            + 0.7 * txZ
            + C
        )
...
def zeroLinkBudget(dist):
    return (
        conf.PTX
        + 2 * conf.GL
        - estimatePathLoss(dist, conf.FREQ)
        - conf.SENSMODEM[conf.MODEM]
    )
