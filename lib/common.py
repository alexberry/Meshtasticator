...
def genScenario():
    ...
    fig.subplots_adjust(
        bottom=0.20, right=0.85
    )  # Make room for button and config
    ...
    button = Button(
        button_ax, "Start simulation", color="red", hovercolor="green"
    )
    # Role selection
    role_ax = fig.add_axes([0.86, 0.61, 0.12, 0.2])
    role_ax.set_axis_off()
    roleButton = RadioButtons(
        role_ax,
        ["Client", "Router", "Repeater"],
        active=1 if conf.router else 0,
    )
    ...
    height_textbox = TextBox(
        height_ax, "Height (m)", conf.HM, textalignment="center"
    )
    ...
    def plotting():
        ...
        ax.set_ylabel("y (m)")
        ax.set_xlim(
            -(conf.XSIZE / 2 + 1) + conf.OX, conf.OX + conf.XSIZE / 2 + 1
        )
        ax.set_ylim(
            -(conf.YSIZE / 2 + 1) + conf.OY, conf.OY + conf.YSIZE / 2 + 1
        )
        ax.set_title(title)
        for i, (nx, ny) in enumerate(zip(nodeX, nodeY)):
            ax.annotate(str(i), (nx - 5, ny + 5))
            circle = plt.Circle(
                (nx, ny), radius=phy.MAXRANGE, color=plt.cm.Set1(i), alpha=0.1
            )
            ax.add_patch(circle)
        if len(nodeTxts) > 0:
...
def findRandomPosition(nodes):
    ...
    while not (foundMin and foundMax):
        ...
        if tries > 1000:
            print(
                "Could not find a location to place the node. Try increasing XSIZE/YSIZE or decreasing MINDIST."
            )
            break
    return x, y
...
def calcDist(x0, x1, y0, y1, z0=0, z1=0):
    return np.sqrt(
        ((abs(x0 - x1)) ** 2) + ((abs(y0 - y1)) ** 2) + ((abs(z0 - z1) ** 2))
    )
...
def plotSchedule(packets, messages):
    def drawSchedule(i):
        t = timeSequences[i]
        plt.suptitle(
            "Time schedule {}/{}\nDouble click to continue.".format(
                i + 1, len(timeSequences)
            )
        )
        ...
        for m in t:  # message generations
            plt.arrow(
                ...
                0,
                0.5,
                head_width=0.02 * (m.endTime - m.genTime),
                head_length=0.3,
                fc="k",
                ec="k",
            )
        ...
        plt.ylabel("Node ID")
        plt.yticks(
            [0] + list(range(conf.NR_NODES)),
            label=[str(n) for n in [0] + list(range(conf.NR_NODES))],
        )
    ...
    timeSequences = [
        t for t in timeSequences if max([m.endTime for m in t]) != 0
    ]
...
class Graph:
    ...
    def save(self):
        ...
        plt.savefig(
            os.path.join("out", "graphics", "placement_" + str(conf.NR_NODES))
        )
