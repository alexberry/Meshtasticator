...
router = (
    False  # set role of each node as router (True) or normal client (False)
)
...
BWMODEM = np.array(
    [250e3, 250e3, 250e3, 250e3, 250e3, 125e3, 125e3, 62.5e3]
)  # bandwidth
...
SENSMODEM = np.array(
    [-121.5, -124.0, -126.5, -129.0, -131.5, -134.5, -137.0, -140.0]
)
# minimum received power for CAD (3dB less than sensitivity)
CADMODEM = np.array(
    [-124.5, -127.0, -129.5, -132.0, -134.5, -137.5, -140.0, -143.0]
)
...
NOISE_LEVEL = (
    -119.25
)  # some noise level in dB, based on SNR_MIN and minimum receiver sensitivity
