...
def setTransmitDelay(node, packet):  # from RadioLibInterface::setTransmitDelay
    for p in reversed(node.packetsAtN[node.nodeid]):
        if (
            p.seq == packet.seq
            and p.rssiAtN[node.nodeid] != 0
            and p.receivedAtN[node.nodeid] == True
        ):
            # verboseprint('At time', round(self.env.now, 3), 'pick delay with RSSI of node', self.nodeid, 'is', p.rssiAtN[self.nodeid])
            return getTxDelayMsecWeighted(
                node, p.rssiAtN[node.nodeid]
            )  # weigthed waiting based on RSSI
    return getTxDelayMsec(node)


def getTxDelayMsecWeighted(
    node, rssi
):  # from RadioInterface::getTxDelayMsecWeighted
    ...
    CWsize = int(
        (snr - SNR_MIN) * (CWmax - CWmin) / (SNR_MAX - SNR_MIN) + CWmin
    )
    if node.isRouter == True:
        ...
        CW = random.randint(0, 2**CWsize - 1)
    verboseprint(
        "Node", node.nodeid, "has CW size", CWsize, "and picked CW", CW
    )
    return CW * slotTime
...
def getTxDelayMsec(node):  # from RadioInterface::getTxDelayMsec
    ...
    CW = random.randint(0, 2**CWsize - 1)
    verboseprint(
        "Current channel utilization is", channelUtil, "So picked CW", CW
    )
    return CW * slotTime


def getRetransmissionMsec(
    node, packet
):  # from RadioInterface::getRetransmissionMsec
    packetAirtime = int(
        airtime(
    ...
    return (
        2 * packetAirtime
        + (2**CWsize + 2 ** (int((CWmax + CWmin) / 2))) * slotTime
