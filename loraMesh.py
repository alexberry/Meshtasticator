...
class MeshNode:
        ...
        while True:
            ...
            if (
                ...
                * airtime(
                ...
                if conf.DMs:
                    destId = random.choice(
                        [
                            i
                            for i in range(0, len(nodes))
                            if i is not self.nodeid
                        ]
                    )
                else:
                ...
                while (
                    p.wantAck
                ):  # ReliableRouter: retransmit message if no ACK received after timeout
                    ...
                    for packetSent in self.packets:
                        if (
                            packetSent.origTxNodeId == self.nodeid
                            and packetSent.seq == p.seq
                        ):
                    ...
                    if ackReceived:
                        verboseprint(
                            "Node",
                            self.nodeid,
                            "received ACK on generated message with seq. nr.",
                            p.seq,
                        )
                        break
                    else:
                        if (
                            minRetransmissions > 0
                        ):  # generate new packet with same sequence number
                            pNew = MeshPacket(
                                self.nodes,
                                self.nodeid,
                                p.destId,
                            ...
                            verboseprint(
                                ...
                                "node",
                                self.nodeid,
                                "wants to retransmit its generated packet to",
                                destId,
                                "with seq.nr.",
                                p.seq,
                                "minRetransmissions",
    ...
    def transmit(self, packet):
        with self.transmitter.request() as request:
            ...
            while (
                any(self.isReceiving)
                or self.isTransmitting
                or isChannelActive(self, self.env)
            ):
                verboseprint(
                    "At time",
                    round(self.env.now, 3),
            ...
            if (
                ...
                verboseprint(
                    ...
                    "node",
                    self.nodeid,
                    "in the meantime received ACK, abort packet with seq. nr",
                    packet.seq,
                )
    ...
    def receive(self, in_pipe):
        ...
        while True:
            p = yield in_pipe.get()
            if (
                p.sensedByN[self.nodeid]
                and not p.collidedAtN[self.nodeid]
                and p.onAirToN[self.nodeid]
            ):  # start of reception
                ...
                verboseprint(
                    ...
                    p.seq,
                    "with delay",
                    round(env.now - p.genTime, 2),
                )
                delays.append(env.now - p.genTime)

                # update hopLimit for this message
                if (
                    p.seq not in self.leastReceivedHopLimit
                ):  # did not yet receive packet with this seq nr.
                ...
                if (
                    p.hopLimit < self.leastReceivedHopLimit[p.seq]
                ):  # hop limit of received packet is lower than previously received one
                    self.leastReceivedHopLimit[p.seq] = p.hopLimit
                ...
                if p.origTxNodeId == self.nodeid:
                    if p.isAck:
                        verboseprint(
                            "Node",
                            self.nodeid,
                            "received real ACK on generated message.",
                        )
                    else:
                        verboseprint(
                            "Node",
                            self.nodeid,
                            "received implicit ACK on message sent.",
                        )
                    p.ackReceived = True
                    continue
                ...
                for sentPacket in self.packets:
                    ...
                    if (
                        sentPacket.txNodeId == self.nodeid
                        and sentPacket.seq == p.seq
                    ):
                        verboseprint(
                            "At time",
                            round(self.env.now, 3),
                    ...
                    if (
                        ...
                        and p.isAck
                        and sentPacket.seq == p.requestId
                    ):
                        verboseprint(
                            "At time",
                            round(self.env.now, 3),
                ...
                if (
                    p.wantAck
                    and p.destId == self.nodeid
                    and not any(pA.requestId == p.seq for pA in self.packets)
                ):
                    verboseprint(
                        "At time",
                        round(self.env.now, 3),
                ...
                # FloodingRouter: rebroadcasting received message
                    ...
                    not p.destId == self.nodeid
                    and not ackReceived
                    and not realAckReceived
                    and p.hopLimit > 0
                ):
                    verboseprint(
                        "At time",
...
nrCollisions = sum(
    [1 for p in packets for n in nodes if p.collidedAtN[n.nodeid] == True]
)
print("Number of collisions:", nrCollisions)
nrSensed = sum(
    [1 for p in packets for n in nodes if p.sensedByN[n.nodeid] == True]
)
print("Number of packets sensed:", nrSensed)
nrReceived = sum(
    [1 for p in packets for n in nodes if p.receivedAtN[n.nodeid] == True]
)
print("Number of packets received:", nrReceived)
meanDelay = np.nanmean(delays)
print("Delay average (ms):", round(meanDelay, 2))
txAirUtilization = (
    sum([n.txAirUtilization for n in nodes])
    / conf.NR_NODES
    / conf.SIMTIME
    * 100
)
...
if nrSensed != 0:
    ...
    print(
        "Percentage of packets that collided:", round(collisionRate * 100, 2)
    )
else:
    print("No packets sensed.")
nodeReach = sum([n.usefulPackets for n in nodes]) / (
    messageSeq * (conf.NR_NODES - 1)
)
print("Average percentage of nodes reached:", round(nodeReach * 100, 2))
if nrReceived != 0:
    usefulness = (
        sum([n.usefulPackets for n in nodes]) / nrReceived
    )  # nr of packets that delivered to a packet to a new receiver out of all packets sent
    print(
